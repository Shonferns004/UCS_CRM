...
import { fetchAttendance, fetchWorkers, updateAttendance, deleteAttendance, addAttendance } from './api'
...
export default function AttendanceView({ readOnly }) {
    ...
    if (selectedWorker) {
      ...
      for (let d = daysInMonth; d >= 1; d--) {
        ...
        const existing = records.find(r => {
          const recordDate = r.punch_in_time ? getIstDateStr(new Date(r.punch_in_time)) : r.date
  ...
  async function saveEdit() {
    ...
    try {
      ...
      await updateAttendance(editingRecord.id, updates)
      await loadData()
      closeEdit()
    } catch (err) {
      alert(err.message)
