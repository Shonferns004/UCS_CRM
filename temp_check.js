...
(async () => {
  ...
  if (ids.length) { const r2 = await c.query('SELECT id, donor_name, donor_id, log_id, agent_name, donor_mobile, receipt_date, project_id, receipt_no FROM receipts WHERE id = ANY($1)', [ids]); console.log('=== linked receipts ==='); console.log(JSON.stringify(r2.rows, null, 2)); }
