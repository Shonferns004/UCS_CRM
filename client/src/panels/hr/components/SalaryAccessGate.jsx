...
// Shared 4-digit access-code gate for salary unlock. open() shows a modal and
// resolves true only when a valid code is entered (or created the first time).
// The code is stored/verified on the server, so the secret never reaches the
// frontend bundle.
export default function useSalaryAccessCode() {
  ...
  const submit = useCallback(async () => {
    ...
    try {
      if (mode === 'create') {
        const res = await apiPost('/salary/access-code', { code });
        if (res && res.ok) return finish(true);
        setError((res && res.message) || 'Could not set the code.');
      } else {
        const res = await apiPost('/salary/access-code/verify', { code });
        if (res && res.ok) return finish(true);
  ...
  const modal = show ? (
    <div
      ...
      style={{
        ...
        padding: 16, animation: 'sacgFade .18s ease',
      }}
    >
      <div
        style={{
      ...
      >
        <div style={{ padding: '22px 24px 16px', borderBottom: '1px solid #eef1f5' }}>
          ...
          <h3 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: '#0f172a' }}>
            {mode === 'create' ? 'Create Salary Access Code' : 'Enter Salary Access Code'}
          </h3>
          <p style={{ margin: '6px 0 0', fontSize: 13, color: '#64748b', lineHeight: 1.5 }}>
            {mode === 'create'
              ...
              : 'Enter the 4-digit access code to unlock salary information.'}
          </p>
        </div>
      ...
      <style>{`
        @keyframes sacgFade { from { opacity: 0; } to { opacity: 1; } }
        @keyframes sacgPop { from { opacity: 0; transform: translateY(8px) scale(.97); } to { opacity: 1; transform: translateY(0) scale(1); } }
      `}</style>
    </div>
  ) : null;
