...
export default function ChangeAccessCode() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!set) {
        const res = await apiPost('/accounts/access-code', { code: next });
        if (res && res.ok) { setSet(true); setSuccess('Access code created.'); resetForm(); }
        else setError('Could not create the code.');
      } else {
        const res = await apiPost('/accounts/access-code/change', { currentCode: current, newCode: next });
  ...
  return (
    <div>
      <p style={{ fontSize: 13, color: '#64748b', margin: '0 0 4px', lineHeight: 1.5 }}>
        Your access code unlocks status reports and downloads. It is unique to your account.
      </p>
      {set == null ? (
