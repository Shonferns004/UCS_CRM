...
export default function BankAudit({embedded,onSummary,selectedEntryId,onSelectEntry,selectionEnabled=true,leadFilter,globalNgo,suspenseNgo:cardSuspenseNgo,onView,amountFilter='',dateFilter='',listRef,onListScroll,onAmounts}){
  ...
  const searchMvDonors=(q)=>{setMvMobile(q);setMvErr('');setMvSearched(false);if(mvTimerRef.current)clearTimeout(mvTimerRef.current);const raw=q.replace(/[^\d]/g,'');if(raw.length<3){setMvResults([]);setMvShowResults(false);return}mvTimerRef.current=setTimeout(async()=>{try{setMvSearching(true);setMvShowResults(true);const r=await apiGet('/accounts/donors?search='+encodeURIComponent(raw));const donors=Array.isArray(r)?r:(r.data||[]);if(donors.length>0){setMvResults(donors);setMvSearched(true)}else{try{const rec=await apiGet('/accounts/receipts/by-mobile?mobile='+encodeURIComponent(raw));if(rec&&rec.donor_name){setMvName(rec.donor_name||'');setMvAddr(rec.address||'');setMvPan(rec.pan_number||'');setMvEmail(rec.email||'');setMvCity(rec.donor_city||'');setMvPinCode(rec.donor_pin_code||'');setMvShowResults(false);setMvSearched(true)}else{setMvResults([]);setMvSearched(true)}}catch(_){setMvResults([]);setMvSearched(true)}}}catch(e){setMvResults([])}finally{setMvSearching(false)}},350)};
  const selectMvDonor=(d)=>{setMvName(d.name||'');setMvMobile(d.mobile_number||d.mobile||d.phone||mvMobile);setMvAddr(d.address_1||d.address||'');setMvPan(d.pan_number||d.pan||'');setMvEmail(d.email||'');setMvCity(d.city||'');setMvPinCode(d.pin_code||d.pincode||'');setMvAddr2(d.address_2||'');setMvResults([]);setMvShowResults(false);setMvSearched(true)};
