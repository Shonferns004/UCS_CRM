...
export default function BankAudit({embedded,onSummary,selectedEntryId,onSelectEntry,selectionEnabled=true,leadFilter,globalNgo,suspenseNgo:cardSuspenseNgo,onView,amountFilter='',dateFilter='',listRef,onListScroll,onAmounts}){
  ...
  const openManualVerify=(entry)=>{setMv(entry);setMvErr('');setMvFro('');setMvType('');setMvMobile(entry.donor_mobile&&entry.donor_mobile!=='NA'?entry.donor_mobile:'');setMvName(entry.donor_name&&entry.donor_name!=='NA'?entry.donor_name:'');setMvAddr(entry.donor_address_1||'');setMvPan(entry.donor_pan||'');setMvEmail(entry.donor_email||'');setMvCity(entry.donor_city||'');setMvPinCode(entry.donor_pin_code||'');setMvAddr2(entry.donor_address_2||'');setMvResults([]);setMvShowResults(false);setShowMvForm(true);};
  ...
  const handleManualVerify=async()=>{if(!mv||mvSub)return;setMvErr('');const mobile=String(mvMobile||'').replace(/[^\d]/g,'');if(mobile.length<10){setMvErr('Please enter a valid donor mobile number');return}if(!mvType){setMvErr('Please select a type (Library, PG, or FRO)');return}setMvSub(true);try{const payload={donor_mobile:mobile,donor_name:mvName||null,donor_address:mvAddr||null,donor_pan:mvPan||null,donor_email:mvEmail||null,donor_city:mvCity||null,donor_pin_code:mvPinCode||null,donor_address_2:mvAddr2||null};if(mvType==='fro'&&mvFro)payload.fro_worker_id=mvFro;if(mvType==='library'||mvType==='pg')payload.project_id=mvType;const res=await apiPost('/accounts/bank-audit/entries/'+mv.id+'/manual-verify',payload);setMv(null);setSe(null);setShowMvForm(false);setMvResults([]);setMvShowResults(false);setTo({msg:res?.message||'Entry manually verified',type:'success',vis:true});await load(sd,st)}catch(e){setMvErr(e.message)}finally{setMvSub(false)}};

  const saveMvDetails=async()=>{if(!mv||mvSub)return;setMvErr('');setMvSub(true);try{await apiPut('/accounts/bank-audit/entries/'+mv.id+'/manual-verify-details',{fro_worker_id:mvFro||null,donor_mobile:mvMobile||null,donor_name:mvName||null,donor_address:mvAddr||null,donor_pan:mvPan||null,donor_email:mvEmail||null,donor_city:mvCity||null,donor_pin_code:mvPinCode||null,donor_address_2:mvAddr2||null});setTo({msg:'Details saved successfully',type:'success',vis:true});await load(sd,st)}catch(e){setMvErr(e.message)}finally{setMvSub(false)}};
      ...
      {st==='unverified'&&(showMvForm||mv)&&(mv||showMvForm)&&<div ref={mvFormRef} style={{margin:'16px 0',padding:'16px',background:'#f8fdf8',border:'1.5px solid #cfe3cb',borderRadius:10}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}>
          <div style={{gridColumn:'1 / -1'}}>
            ...
            <select value={mvType} onChange={e=>{setMvType(e.target.value);setMvFro('');setMvErr('')}} style={{width:'100%',padding:'9px 12px',borderRadius:8,border:'1.5px solid #e5e7eb',fontSize:13,boxSizing:'border-box',outline:'none',background:'#fff',cursor:'pointer'}}>
          ...
          {mvType==='fro'&&<div style={{gridColumn:'1 / -1'}}>
            <label style={{display:'block',fontSize:11,fontWeight:600,color:'#6b7280',marginBottom:5,textTransform:'uppercase',letterSpacing:'.4px'}}>FRO <span style={{color:'#9ca3af',fontWeight:400,textTransform:'none',letterSpacing:0}}>— optional</span></label>
             <FroSearchPicker value={mvFro} fros={wr.filter(w=>String(w.department||'').toLowerCase()==='fro')} onChange={id=>{setMvFro(id);setMvErr('')}}/>
          </div>}
