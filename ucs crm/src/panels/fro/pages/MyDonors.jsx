          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            ...
            <div style={{ margin: '10px 0', borderRadius: 10, overflow: 'hidden', display: 'flex', gap: 6 }}>
              ...
              <button onClick={(e) => { e.stopPropagation(); saveFormState(); saveViewState(); saveProgress(dataTab, donor.id, index); localStorage.setItem(`${dataTab}_${stationKey}_donor_progress`, JSON.stringify({ id: donor.id, idx: index })); navigate(getWhatsAppChatUrl(donor)) }}
                title="WhatsApp"
                style={{ width: 58, border: 'none', borderRadius: 12, background: 'linear-gradient(135deg, #25D366 0%, #128C7E 100%)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, flexDirection: 'column', gap: 2, boxShadow: '0 2px 8px rgba(37,211,102,.35)', transition: 'transform .15s, box-shadow .15s' }}
                onMouseEnter={e => { e.currentTarget.style.transform = 'scale(1.05)'; e.currentTarget.style.boxShadow = '0 5px 16px rgba(37,211,102,.55)'; }}
                onMouseLeave={e => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.boxShadow = '0 2px 8px rgba(37,211,102,.35)'; }}>
