      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '2px 18px 18px' }}>
        {list.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {list.map(r => {
              const badge = r.waiting_receipt_no ? { text: 'Waiting for receipt number', color: '#6b7280', bg: '#f3f4f6' } : r.kind === 'no_receipt' ? { text: 'Awaiting Receipt', color: '#6b7280', bg: '#f3f4f6' } : r.kind === 'receipt_sent' ? CLAIM_BADGES.receipt_sent : CLAIM_BADGES[r.my_claim_status];
              const claimable = !r.waiting_receipt_no && r.kind !== 'no_receipt' && (!r.my_claim_status || r.kind === 'receipt_sent');
              return (
