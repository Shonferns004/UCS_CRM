              ...
              {paginatedDonors.map(d => (
                <tr key={`${d.id}-${d.ngo_id}`} onClick={() => openModal(d)}
                  ...
                  <td style={{ padding: '8px 10px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    {d.donor_mobile && (
                      <button onClick={e => { e.stopPropagation(); localStorage.setItem('donors_last_view', JSON.stringify({ page, perPage, search, filter, period })); navigate(getWhatsAppChatUrl(d)) }}
