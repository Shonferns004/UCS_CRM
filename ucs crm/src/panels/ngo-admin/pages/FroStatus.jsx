...
export default function FroStatus() {
  const { user } = useUcs()
  ...
  const resetAllIdle = async () => {
    ...
    setResetting(true)
    try {
      await apiGet('/fro/status/reset-idle', { method: 'PUT', body: JSON.stringify({}) })
      await loadStatuses()
    } catch (e) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        ...
        <div style={{ flex: 1 }}>
          <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>FRO Status</h3>
        ...
        {user?.role === 'super_admin' && (
          <button
            ...
            disabled={resetting}
            style={{ fontSize: 11, padding: '6px 12px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: 6, cursor: resetting ? 'wait' : 'pointer', fontFamily: 'inherit', fontWeight: 600 }}
          >
