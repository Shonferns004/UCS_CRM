...
export default function NgoAdminPanel() {
  ...
  if (!seenNotifIds.current) {
    try { seenNotifIds.current = new Set(JSON.parse(localStorage.getItem('ngoadmin_seen_notifs') || '[]')); } catch { seenNotifIds.current = new Set(); }
  }
  ...
  return (
    <div className="app">
              ...
              <div style={{ display:'flex', gap:4, alignItems:'center', background:'var(--card-bg)', borderRadius:8, border:'1px solid var(--line)', padding:'4px 8px' }}>
                ...
                {searchQuery && (
                  <span className="material-symbols-outlined" style={{ fontSize:14, color:'var(--ink-soft)', cursor:'pointer' }}
                    onClick={() => { setSearchQuery(''); setSearchResults({ donors:[], fros:[], stations:[] }); setShowSearchDropdown(false); }}>close</span>
                )}
      ...
      {selectedResult && (() => {
        ...
        const iconMap = { donor: 'person', fro: 'group', station: 'home' }
